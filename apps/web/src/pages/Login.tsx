    ...
    <div className="relative grid min-h-screen grid-cols-1 bg-bg text-text lg:grid-cols-[1fr_460px]">
      ...
      <section className="flex flex-col border-l border-border bg-panel px-12 py-11">
        ...
        <div className="flex max-w-[340px] flex-1 flex-col justify-center">
          ...
          {/* Entrar ≠ conectar (SPEC-026). A caixa existe para o usuário não
              esperar que o login já dê acesso aos repositórios — a conexão
              GitHub é um segundo ato, dentro do painel. */}
          <div className="mt-5 flex items-start gap-2.5 rounded-[10px] border border-border2 bg-surface px-4 py-3.5">
            ...
            <span className="text-[12.5px] leading-[1.55] text-muted">
              Sua conta Google identifica{' '}
              <strong className="font-semibold text-body2">quem dá o aceite</strong>. A conexão
              com o GitHub é feita depois, dentro do painel, e é só para leitura de
              documentação.
            </span>
          </div>
...
/**
 * "G" oficial do Google. As cores são fixas nos dois temas de propósito — as
 * diretrizes da marca não permitem recolorir o logo, então ele não usa os
