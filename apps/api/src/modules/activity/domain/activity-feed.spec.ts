...
describe('buildFeed — projeção do histórico de Atividade', () => {
  it('unifica as 4 fontes em ordem reversa por data', () => {
    const feed = buildFeed({
      ...
      insights: [
        { id: 'i1', kind: 'architecture_fallback', provider: 'anthropic', model: 'claude-sonnet-5', createdAt: d('2026-07-13T09:00:00Z') },
      ],
      mutations: [
  ...
  it('insight: mostra provider/model no detalhe, sem link', () => {
    const [item] = buildFeed({
      ...
      insights: [{ id: 'i1', kind: 'summary', provider: 'anthropic', model: 'x', createdAt: d('2026-07-13T10:00:00Z') }],
    });
  ...
  it('nenhum título tem jargão técnico', () => {
    const feed = buildFeed({
      ...
      insights: [{ id: 'i1', kind: 'edges_marker', provider: 'a', model: 'm', createdAt: d('2026-07-13T09:00:00Z') }],
      mutations: [],
