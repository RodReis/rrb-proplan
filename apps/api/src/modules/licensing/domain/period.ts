/**
 * O período das contagens de licenciamento (SPEC-040 §Métricas).
 *
 * **Lista fechada, padrão 30.** Valor fora dela é recusado pela rota, nunca
 * corrigido em silêncio para o padrão (§6 da SPEC-035, que vale aqui inteiro):
 * corrigir caladamente faria um erro de front virar dado errado em tela, e o
 * número apareceria plausível.
 *
 * ## Por que este arquivo é uma cópia do `dashboard/domain/period.ts`
 *
 * As regras são as mesmas — a lista fechada e a virada em BRT — mas o
 * `licensing` é **frente disjunta** (MVP4 §3, mesmo princípio dos ADR-023/024),
 * e a `licensing-boundaries.arch.spec` varre justamente importações de outros
 * módulos. Importar de `dashboard/` criaria a dependência que a arch-spec
 * existe para impedir, e por um arquivo de 60 linhas sem estado.
 *
 * **O que a duplicação custa, e por que é o custo certo:** se o Brasil voltar a
 * ter horário de verão, são dois arquivos a mudar em vez de um — e os testes
 * dos dois falham antes de qualquer contagem mentir, que é o comportamento
 * desejado. O caminho de volta, se um terceiro módulo precisar disto, é
 * extrair para um lugar comum com a bênção do arch-spec dos três.
 */
