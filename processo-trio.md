<!--
  ...
  Adapte antes de usar:
    - paths de docs, se seu repo usar outra convenção;
    - troque cada [ADR-XXX] pelo ADR correspondente no seu docs/DECISIONS.md
      (as regras marcadas assim precisam de respaldo documentado — sem isso,
      viram regra órfã que alguém questiona depois).
-->
...
> **Projeção do épico no board:** a projeção do ProPlan é `issue → coluna` por **label + open/closed**. O **MVP-épico não é card de fatia** — ele existe para agrupar, e sua ligação com as filhas vive no checklist do corpo (e na relação de sub-issue, quando o ProPlan a lê). Como o épico deve aparecer visualmente no board é **decisão de produto do PI e escopo do próprio ProPlan** — trate como fatia dedicada do ProPlan, não como algo a improvisar aqui. Até essa fatia existir, o épico **não** entra na projeção de colunas.
...
| entrega | **Code** | abre PR com **`refs #N`** no corpo. **NUNCA `closes #N`** — fecharia a issue no merge e **forjaria o aceite do PI** [ADR-XXX]. Só **depois do merge**, aplica `proplan:done` → **Feito**, com o link do PR no corpo da issue |
...
Issue nunca é deletada. `closes #N` é proibido; usar sempre `refs #N` (razão na tabela acima). Mover para Finalizado/Descartado posta comentário de carimbo na issue.
