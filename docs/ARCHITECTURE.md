...
| **Identity** | **Conta ⊥ conexão** (SPEC-026): a **sessão do app** deriva da identidade — autenticada por um **IdP** (Google, `/auth/google`) — e **não** de nenhuma conexão. O **GitHub é conexão** (ADR-015): OAuth do App → `userToken` (**todas as leituras**); JWT RS256 → `installationToken` cacheado (**todas as escritas**, identidade `proplan[bot]`). Os tokens da conexão vivem na entidade **`Connection`** (`userId` + `provider`, único por par — SPEC-025), **nunca no `User`**: é o que permite revogar a conexão sem tocar a sessão. Ausência de linha = sem conexão, e isso é informação, não falha (o catálogo a lê como *conecte o GitHub*). Membership de tenant (ADR-020) deriva do `userId` da identidade. Multi-tenant e RBAC na Fatia 8 | Deixar leitura usar installation token (vazaria repos que o usuário logado não enxerga); **derivar sessão ou membership de uma conexão** — perder o GitHub não pode deslogar; guardar token de conexão no `User` |
