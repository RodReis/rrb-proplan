...
**E isso não é bug do promote.** É a forma de **toda** escrita do ProPlan:

> **ação → commit no GitHub → propagação → sync → recarregar**

Quatro passos, sempre os mesmos, sempre invisíveis, em **quatro fluxos**: promote (Fatia 7), bootstrap (Fatia 3), mutação de card no Kanban (Fatia 5) e salvar mapeamento `.proplan/config.yml` (Fatia 6). Consertar só o promote é remendar um sintoma de quatro.
...
- Cada operação tem: `kind` (promote · bootstrap · board_mutation · mapping), `steps[]` com estado (`pending|running|done|failed`), `startedAt`, `finishedAt?`, `error?`, e os **artefatos que produziu** (URL do commit, `syncRunId`).
...
- Os **quatro fluxos existentes migram** para ele. Nenhum mantém feedback próprio.
...
- [ ] **Os quatro fluxos** (promote, bootstrap, mutação de card, salvar mapeamento) usam o **mesmo** componente. Nenhum tem feedback próprio.
...
- Prisma: `Operation { id, projectId, kind, status, steps Json, error?, commitUrl?, syncRunId?, startedAt, finishedAt? }`. As tabelas existentes (`SyncRun`, `Insight`, `BoardMutation`, e `LlmUsage` da 7.5) **continuam sendo a fonte** — `Operation` amarra os passos de uma ação do usuário; o **histórico é composto por leitura das quatro**, não duplicado.
- API: `POST` dos fluxos existentes passa a devolver `{ operationId }` · `GET /operations/:id` (estado + passos) · `GET /projects/:id/activity?cursor=` (histórico paginado).
