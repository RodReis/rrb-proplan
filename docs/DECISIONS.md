...
3. **A regra é verificada por máquina, não por revisão.** Import profundo em `modules/llm/**` a partir de fora quebra o build (regra de lint com `no-restricted-imports`, checada no CI). Regra de fronteira que depende de alguém lembrar é regra que sobrevive até o primeiro PR apressado — mesma disciplina do teste que barra `closes #N` e da auditoria de RLS.
