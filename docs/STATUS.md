...
### Não priorizado
- **Documentos ricos — árvore + binários** (achado no aceite runtime da Fatia 6, 2026-07-13): (a) a lista de docs vira **árvore de pastas** (hoje é plana; com repos de muitas subpastas o scroll fica enorme e a hierarquia some) — UX pura, tratada como polimento da aba Documentos; (b) **preview de binários** `.pdf` (iframe nativo), `.docx` (extrair texto), `.html` (render sandboxed) — hoje aparecem como lixo binário/HTML cru porque o pipeline os lê como texto. O (b) **exige emenda ao ADR-003** (binário em `docs/` não viola o path, mas o storage `content: text` não comporta binário — decidir buscar sob demanda vs. bytea; e sandbox de HTML é superfície de XSS) e **spec própria** antes de codar (prio: média — decisão do PI de fazer agora; sem spec ainda)
