...
## Fatia 7.7 — Invalidação de inferência por `inputHash` (SPEC-011, `aprovada-pi`) — `feito` (aguardando merge + aceite runtime do PI)
...
**Achado técnico (para o aceite):** a spec tem uma **contradição interna** — Escopo item 2 diz "hit ⇒ atualiza `docsTreeSha` para o corrente"; a Decisão 2 do PID (2026-07-14) diz o oposto — "hit **não** sobrescreve, vira metadado histórico". Segui a **Decisão 2** (mais recente, datada, "verificado no código"). Se o PI quis o contrário, é um `UPDATE docs_scope_hash` a mais no hit.
