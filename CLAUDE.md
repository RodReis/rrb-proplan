...
- **Estado do trabalho vive nas GitHub Issues** (ADR-011). **6 colunas**: `open`+`proplan:backlog|todo|doing` · **Feito** = `closed` sem label (*entregue, aguardando aceite*) · **Finalizado** = `closed`+`proplan:finalizado` (*aceito pelo PI*) · **Descartado** = `closed`+`proplan:descartado`. Issue nunca é deletada. `closes #42` num PR cai em **Feito**, nunca em Finalizado — aceite é ato deliberado do PI, nenhuma automação o forja. Mover para Finalizado/Descartado posta comentário de carimbo na issue. Repo sem Issues → board somente leitura (modo degradado, sinalizado na UI).
